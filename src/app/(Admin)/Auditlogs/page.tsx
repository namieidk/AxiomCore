...
export default function AuditLogsPage() {
  ...
  }, [mode, page, moduleFilter, fetchActivityLogs, fetchLoginLogs]);
  ...
  return (
    <main className="h-screen w-full flex flex-col lg:flex-row bg-[#020617] text-slate-200 overflow-hidden font-sans uppercase relative">
      ...
      <section className="flex-1 flex flex-col overflow-hidden bg-[radial-gradient(ellipse_at_top_left,_var(--tw-gradient-stops))] from-indigo-900/10 via-[#020617] to-[#020617]">
        ...
        <header className="px-6 lg:px-12 py-6 lg:py-10 border-b border-white/5 flex flex-col md:flex-row justify-between items-start md:items-end gap-6 backdrop-blur-md bg-[#020617]/80 shrink-0">
          <div>
            <div className="flex items-center gap-2 text-indigo-500 mb-2">
              ...
              <span className="text-[8px] lg:text-[10px] font-black uppercase tracking-[0.4em]">Audit Trail Protocol</span>
            </div>
          ...
          <div className="flex items-center gap-4 w-full md:w-auto">
            <button
              ...
              className="p-3 bg-white/5 border border-white/5 rounded-xl hover:bg-indigo-500/10 transition-all group"
            >
            ...
            <div className="flex flex-1 md:flex-none bg-white/5 p-1.5 rounded-2xl border border-white/5 gap-2">
              <button
                ...
                className={`flex-1 md:flex-none px-4 lg:px-8 py-3 rounded-xl text-[9px] lg:text-[10px] font-black tracking-widest transition-all ${
        ...
        {mode === 'activities' && (
          <div className="px-6 lg:px-12 py-5 border-b border-white/5 flex items-center gap-3 shrink-0 overflow-x-auto scrollbar-hide">
            ...
            {MODULES.map((mod) => (
              <button
                ...
                className={`px-4 py-1.5 rounded-full text-[8px] font-black tracking-widest border transition-all shrink-0 ${
                  moduleFilter === mod ? MODULE_COLORS[mod] : 'text-slate-600 border-white/5 bg-white/5 hover:text-slate-300'
                }`}
              >
                {mod}
              </button>
        ...
        <footer className="px-6 lg:px-12 py-5 border-t border-white/5 bg-[#020617] flex justify-between items-center shrink-0 pb-20 lg:pb-5">
          <div className="flex items-center gap-6">
            <p className="hidden md:block text-[8px] font-black text-slate-600 tracking-[0.4em]">LOG RECORDS ARE IMMUTABLE</p>
            <div className="flex items-center gap-2 px-3 py-1 bg-white/5 rounded-full border border-white/5">
              <Database className="w-3 h-3 text-indigo-500" />
              <span className="text-[8px] font-black text-slate-500 tracking-widest uppercase">Vault: Active</span>
            </div>
          </div>
          <div className="flex items-center gap-4 text-indigo-600">
            <Cpu className="w-3 h-3" />
            <span className="hidden sm:inline text-[8px] font-black tracking-widest uppercase">Integrity: Nominal</span>
