    ...
    <main className="h-screen w-full flex bg-[#020617] text-slate-200 overflow-hidden font-sans uppercase italic">
      ...
      <section className="flex-1 flex flex-col overflow-y-auto bg-[radial-gradient(circle_at_bottom_right,_var(--tw-gradient-stops))] from-blue-900/10 via-[#020617] to-[#020617] custom-scrollbar">
        ...
        <div className="p-6 md:p-12 max-w-[1600px] w-full mx-auto space-y-8 md:space-y-12">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 md:gap-10">
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-3 md:space-y-4">
                {isLoading ? (
                  [1,2,3,4].map(i => <div key={i} className="bg-slate-900/40 border border-white/5 p-6 rounded-[1.5rem] animate-pulse h-20" />)
                ) : archiveItems.map((report) => {
                  const Icon = typeIcon(report.type);
                  return (
                    <div key={report.id} className="bg-slate-900/40 border border-white/5 p-4 md:p-6 rounded-[1.5rem] md:rounded-[2rem] flex items-center justify-between group hover:border-blue-500/30 transition-all cursor-pointer">
                      <div className="flex items-center gap-3 md:gap-4 min-w-0">
                        <div className="p-2.5 md:p-3 rounded-xl bg-white/5 text-slate-500 group-hover:text-blue-500 shrink-0"><Icon className="w-4 h-4 md:w-5 md:h-5" /></div>
          ...
          {/* LEAVE QUEUE - Fully Responsive Table/List */}
          <div className="bg-slate-900/20 border border-white/5 rounded-[2rem] md:rounded-[3.5rem] overflow-hidden backdrop-blur-3xl mb-12 shadow-2xl">
            ...
            <div className="overflow-x-auto custom-scrollbar">
              {leaveLoading ? (
                <div className="p-6 md:p-10 space-y-4">{[1,2,3].map(i => <div key={i} className="h-20 bg-slate-800/40 rounded-xl md:rounded-2xl animate-pulse" />)}</div>
              ) : leaveRequests.length === 0 ? (
                    ...
                    <div key={req.id} className="px-6 md:px-10 py-5 md:py-6 flex items-center justify-between hover:bg-white/[0.02] transition-all group">
                      <div className="flex items-center gap-4 md:gap-5 min-w-0">
                        ...
                        <div className="min-w-0">
                          ...
                          <div className="flex items-center gap-3 text-[7px] md:text-[8px] font-black text-slate-500 tracking-widest uppercase truncate">
                            <span className="text-blue-400">{req.type}</span><span>•</span><span>{req.date}</span><span>•</span><span className="truncate max-w-[200px] md:max-w-[300px]">{req.reason}</span>
                          </div>
                        </div>
                      </div>
                      <div className="flex items-center gap-2 md:gap-3 shrink-0 ml-4">
                        {actionId === req.id ? (
                          ...
                          <>
                            <button onClick={() => onLeaveAction(req.id, 'APPROVED')} className="px-4 md:px-5 py-2 md:py-2.5 rounded-lg md:rounded-xl bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 hover:bg-emerald-500 hover:text-white text-[7px] md:text-[8px] font-black tracking-widest transition-all shadow-lg hover:shadow-emerald-500/20 active:scale-95">
                              <Check className="w-3 md:w-3.5 h-3 md:h-3.5 inline mr-1 md:mr-1.5" /> APPROVE
                            </button>
                            <button onClick={() => onLeaveAction(req.id, 'REJECTED')} className="px-4 md:px-5 py-2 md:py-2.5 rounded-lg md:rounded-xl bg-red-500/10 border border-red-500/20 text-red-400 hover:bg-red-500 hover:text-white text-[7px] md:text-[8px] font-black tracking-widest transition-all shadow-lg hover:shadow-red-500/20 active:scale-95">
