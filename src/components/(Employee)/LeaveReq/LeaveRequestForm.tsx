...
export const LeaveRequestForm = ({
  ...
  // Emergency: no min restriction on dates. Others: today is the minimum.
  const startMin = isEmergency ? undefined : todayLimit;
  const endMin   = isEmergency ? undefined : (dates.start || todayLimit);
    ...
    <section className="relative flex-1 flex flex-col overflow-y-auto bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-indigo-900/20 via-[#050510] to-[#050510] scrollbar-hide uppercase italic font-black">
      ...
      <div className="p-6 lg:p-12 max-w-[1200px] w-full mx-auto space-y-8 lg:space-y-10">
        ...
        <form onSubmit={onSubmit} className="bg-indigo-950/20 border border-indigo-500/10 rounded-[2rem] lg:rounded-[3.5rem] p-6 lg:p-12 shadow-2xl backdrop-blur-3xl space-y-8 lg:space-y-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 lg:gap-10 items-start">
            ...
            <div className="space-y-3 lg:space-y-4">
              ...
              <select
                ...
                className="w-full h-[56px] lg:h-[62px] bg-indigo-950/40 border border-indigo-500/10 rounded-2xl px-6 text-[10px] font-black text-white outline-none focus:border-indigo-500/50 appearance-none cursor-pointer"
              >
              ...
              {isEmergency && (
                <p className="text-[8px] font-black text-amber-400/80 tracking-widest ml-1 uppercase not-italic">
                  ⚠ Emergency leave allows past dates · Max 3 days
                </p>
              )}
            </div>
          ...
          {/* REASON — normal-case so typed text is not forced uppercase */}
          <div className="space-y-3 lg:space-y-4">
            ...
            <textarea
              ...
              className="w-full h-32 lg:h-40 bg-indigo-950/40 border border-indigo-500/10 rounded-[1.5rem] lg:rounded-[2.5rem] p-6 lg:p-8 text-[11px] lg:text-xs font-black text-white outline-none focus:border-indigo-500/50 resize-none normal-case placeholder:opacity-30"
            />
          </div>
          ...
          <div className="flex flex-col sm:flex-row justify-between items-center pt-8 border-t border-indigo-500/10 gap-6">
            <div className="flex items-center gap-3 text-indigo-400">
              ...
              <span className="text-[8px] lg:text-[9px] font-black uppercase tracking-widest">
                ...
                {isEmergency && requestedDays > 0 && (
                  <span className="text-amber-400/80 ml-2">· 2–3 Day Allocation</span>
                )}
              </span>
            </div>
            <button
              type="submit"
              className="w-full sm:w-auto px-12 py-5 bg-indigo-600 text-white rounded-2xl font-black text-[10px] tracking-[0.3em] hover:bg-indigo-500 transition-all shadow-xl active:scale-95 italic border border-indigo-400/20"
            >
              Apply Request
            </button>
          </div>
        ...
        <div className="space-y-6 pb-20">
          ...
          <div className="bg-indigo-950/20 border border-indigo-500/10 rounded-[1.5rem] lg:rounded-[3rem] overflow-hidden backdrop-blur-xl">
            {history.length > 0 ? history.map((log, i) => (
              ...
              <div className="p-16 lg:p-20 text-center text-[9px] lg:text-[10px] font-black text-indigo-900 tracking-[0.5em] uppercase">
